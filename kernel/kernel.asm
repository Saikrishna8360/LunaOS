...
; desiredFrequency: EQU 100   ; Desired interrupt frequency (in Hz)
...
; Calculate divisor
    mov rax, 1193182            ; Base frequency of PIT (approximately 1.193182 MHz)
    mov rax, desiredFrequency
    mov bx, ax  ; Load desired interrupt frequency into BX
    div bx                     ; Divide base frequency by desired frequency
    mov dx, 0                  ; Clear DX (upper 16 bits of dividend)
    mov bx, ax                 ; Store quotient (divisor) in BX
 
; Set PIT divisor
    mov al, bl                 ; Lower byte of divisor
    out 0x40, al               ; Output lower byte to PIT channel 0 data port
    mov al, bh                 ; Upper byte of divisor
    out 0x40, al               ; Output upper byte to PIT channel 0 data port
